...
app.config['MAIL_USERNAME'] = 'returnright.info@gmail.com'
app.config['MAIL_PASSWORD'] = "rqup ggrr zcmg bqjh"
app.config['MAIL_DEFAULT_SENDER'] = 'returnright.info@gmail.com'
mail = Mail(app)
db = SQLAlchemy(app)
...
def login():
    if request.method == 'POST':
        ...
        if user and user.password==password:  # Use hashed password
            session['user_id'] = user.id
            return redirect(url_for('index'))
        else:
